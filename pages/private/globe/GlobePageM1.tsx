...
import { SEVEN_SUMMITS, Summit } from '../../../components/ui/RotatingGlobe';
import EarthGlobe, { EarthGlobeHandle, subsolarPoint } from './EarthGlobe';

// Unlisted mockup: the live #globe page (pages/GlobePage.tsx) with the M1
// photo-real globe in place of the D3 dotted globe, to judge it in context.
// Everything else on the page is copied unchanged.
...
const GlobePageM1: React.FC = () => {
  ...
  const handleSummitClick = (summit: Summit) => {
    ...
    globeRef.current?.flyTo(summit.lat, summit.lng, { distance: 2.05, duration: 2200 });
  };
  ...
  return (
    <div className="bg-brand-dark min-h-screen">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            ...
            <div className="lg:col-span-2 order-1 lg:order-2">
              {/* Fixed-size round frame; zooming changes the view inside it, not its size */}
              <div className="flex justify-center">
                <div className="relative aspect-square w-[min(100%,62vh)] overflow-hidden rounded-full shadow-[0_0_80px_rgba(56,140,255,0.25)]">
                  <EarthGlobe
                    ref={globeRef}
                    className="absolute inset-0"
                    markers={SEVEN_SUMMITS.map((s) => ({ id: s.id, lat: s.lat, lng: s.lng }))}
                    activeMarker={activeSummit}
                    autoRotate={!activeSummit}
                    fitRadius={1.16}
                    minZoomDistance={1.35}
                    initialView={initialView}
                    onMarkerClick={(id) => {
                      const summit = SEVEN_SUMMITS.find((s) => s.id === id);
                      if (summit) handleSummitClick(summit);
                    }}
                  />
                </div>
              </div>
              ...
              <div className="mt-4 text-center text-sm text-gray-500">
                Drag to rotate &bull; Scroll to zoom inside the frame &bull; Click a summit to fly there
              </div>
      ...
      <a
        href="#private"
        className="fixed bottom-4 right-4 z-50 rounded-full border border-amber-400/40 bg-black/80 px-3 py-1.5 text-[11px] font-semibold uppercase tracking-wider text-amber-300"
      >
        Mockup · M1 in the globe page · ← Sandbox
      </a>
